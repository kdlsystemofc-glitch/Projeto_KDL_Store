  ...
  useEffect(() => {
    ...
    async function load() {
      const [tx, pay, rec] = await Promise.all([
        supabase.from('cash_transactions').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(50),
        supabase.from('accounts_payable').select('*').eq('tenant_id', tenantId).order('due_date'),
        supabase.from('accounts_receivable').select('*, customers(name)').eq('tenant_id', tenantId).order('due_date'),
      ]);
