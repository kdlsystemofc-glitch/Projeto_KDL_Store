...
export default function GarantiasPage() {
  ...
  async function openClaim(w: Warranty) {
    ...
    const payload: any = {
      ...
      warranty_id: w.id,
      status: 'approved',
      description: `Garantia acionada — ${w.products?.name}`,
      price: 0,
    };
    ...
    if (viewing?.id === w.id) setViewing({ ...viewing, status: 'claimed' });
  }

  async function saveWarranty(e: React.FormEvent) {
    e.preventDefault();
    if (!form.product_id || !form.customer_id) {
      alert('Selecione produto e cliente');
      return;
    }
    setSaving(true);
    ...
    let attempts = 0;
    while (!finalData && attempts < 10) {
      attempts++;
      const res = await supabase.from('warranties')
        .insert(payload)
        .select('*, products(name), customers(name,phone)')
        .single();
      ...
      if (res.error) {
        const match = res.error.message.match(/'([^']+)' column/);
        if (match?.[1]) { delete payload[match[1]]; continue; }
        alert('Erro ao emitir garantia: ' + res.error.message);
        setSaving(false);
        return;
      }
      finalData = res.data;
    ...
    if (finalData) {
      setList(prev => [...prev, finalData as Warranty].sort((a, b) => a.expiry_date.localeCompare(b.expiry_date)));
      setShowModal(false);
      setForm({ product_id: '', customer_id: '', warranty_months: 12, issue_date: new Date().toISOString().split('T')[0], notes: '' });
    }
    setSaving(false);
  }
  ...
  function WarrantyCard({ w }: { w: Warranty }) {
    ...
    return (
      <div
        style={{ background: 'var(--kdl-surface)', border: '1px solid var(--kdl-border)', borderRadius: 12, padding: '1.25rem', transition: 'all 0.2s', cursor: 'pointer' }}
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ fontSize: '0.72rem', color: 'var(--kdl-text-dim)' }}>
              Emitida {new Date(w.issue_date).toLocaleDateString('pt-BR')} · {w.warranty_months}m
            </p>
          ...
          {w.status === 'active' && (
            <button
              ...
              onClick={e => { e.stopPropagation(); openClaim(w); }}
            >⚡ Acionar</button>
          )}
        </div>
        {w.warranty_code && (
          <p style={{ fontSize: '0.65rem', fontFamily: 'monospace', color: 'var(--kdl-text-dim)', marginTop: 8 }}>
            {w.warranty_code}
          </p>
        )}
      </div>
        ...
        : !filtered.length
          ? (
            <div className="empty-state">
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--kdl-text-muted)' }}>
                Garantias são emitidas ao finalizar uma venda ou manualmente pelo botão acima.
              </p>
            </div>
          )
          : (
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={saveWarranty} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="gar-notes">Observações</label>
                <input
                  id="gar-notes"
                  type="text"
                  className="form-input"
                  ...
                  placeholder="Termos especiais, número de série, etc."
                />
              </div>
              {form.warranty_months > 0 && form.issue_date && (
                <div className="alert alert-info">
                  📅 Vence em: <strong>
                    {new Date(new Date(form.issue_date).setMonth(new Date(form.issue_date).getMonth() + form.warranty_months)).toLocaleDateString('pt-BR')}
                  </strong>
                </div>
              )}
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                <button type="button" className="btn btn-secondary" onClick={() => setShowModal(false)}>Cancelar</button>
                <button id="gar-save" type="submit" className="btn btn-primary" disabled={saving}>
                  {saving ? 'Emitindo...' : '🛡️ Emitir Garantia'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setViewing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              {viewing.warranty_code && (
                <p style={{ fontFamily: 'monospace', color: 'var(--kdl-primary-light)', fontSize: '1rem', marginTop: 4, letterSpacing: 2 }}>
                  {viewing.warranty_code}
                </p>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: '1.5rem' }}>
              {viewing.customers?.phone && (
                <a
                  href={`https://wa.me/55${viewing.customers.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Olá ${viewing.customers.name}! Sua garantia digital:\n\nProduto: ${viewing.products?.name}\nCódigo: ${viewing.warranty_code || viewing.id.slice(0,8).toUpperCase()}\nValidade: ${new Date(viewing.expiry_date).toLocaleDateString('pt-BR')}`)}`}
                  target="_blank"
                  rel="noreferrer"
