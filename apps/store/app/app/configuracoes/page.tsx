...
export default function ConfiguracoesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return;
      const { data: ud } = await supabase.from('users').select('tenant_id, tenants(name, slug, status, plans(name, price_monthly))').eq('id', user.id).single();
      if (!ud) return;
      const t = ud.tenants as any;
      const p = t?.plans;
  ...
  async function saveTenant(e: React.FormEvent) {
    ...
    setSaving(false); setSavedMsg('Salvo com sucesso!');
  ...
  const ROLES: Record<string, string> = { owner: 'Dono', manager: 'Gerente', seller: 'Vendedor', technician: 'Técnico' };
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem' }}>
        {(['loja', 'usuarios', 'assinatura'] as const).map(t => (
          <button key={t} id={`cfg-tab-${t}`} onClick={() => setTab(t)} className={`btn ${tab === t ? 'btn-primary' : 'btn-secondary'}`}>
        ...
        <div className="card" style={{ maxWidth: 520 }}>
          ...
          <form onSubmit={saveTenant} id="loja-form" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="form-group">
              ...
              <input id="loja-name" type="text" className="form-input" value={storeForm.name} onChange={e => setStoreForm(f => ({ ...f, name: e.target.value }))} required />
            </div>
            <div className="form-group">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <button id="loja-save" type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Salvando...' : 'Salvar Alterações'}</button>
      ...
      {tab === 'usuarios' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            ...
            <button id="usr-novo-btn" className="btn btn-primary" onClick={() => setShowUserModal(true)}>+ Adicionar Usuário</button>
          </div>
          <div className="table-wrapper">
            <table>
              <thead><tr><th>Nome</th><th>Email</th><th>Perfil</th><th style={{ textAlign: 'center' }}>Status</th><th style={{ textAlign: 'center' }}>Ações</th></tr></thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id}>
                    <td style={{ fontWeight: 600 }}>{u.name || '—'}</td>
                    <td style={{ color: 'var(--kdl-text-muted)' }}>{u.email}</td>
                    <td><span className="badge badge-info">{ROLES[u.role] || u.role}</span></td>
                    <td style={{ textAlign: 'center' }}><span className={`badge ${u.is_active ? 'badge-success' : 'badge-gray'}`}>{u.is_active ? 'Ativo' : 'Inativo'}</span></td>
                    <td style={{ textAlign: 'center' }}>
                      {u.role !== 'owner' && (
                        <button className={`btn btn-sm ${u.is_active ? 'btn-danger' : 'btn-success'}`} onClick={() => toggleUser(u)}>
            ...
            <div className="modal-overlay" onClick={() => setShowUserModal(false)}>
              <div className="modal" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={addUser} id="user-form" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div className="form-group"><label className="form-label" htmlFor="usr-name">Nome *</label><input id="usr-name" type="text" className="form-input" value={userForm.name} onChange={e => setUserForm(f => ({ ...f, name: e.target.value }))} required /></div>
                  <div className="form-group"><label className="form-label" htmlFor="usr-email">Email *</label><input id="usr-email" type="email" className="form-input" value={userForm.email} onChange={e => setUserForm(f => ({ ...f, email: e.target.value }))} required /></div>
                  <div className="form-group"><label className="form-label" htmlFor="usr-pass">Senha temporária *</label><input id="usr-pass" type="password" className="form-input" value={userForm.password} onChange={e => setUserForm(f => ({ ...f, password: e.target.value }))} required /></div>
                  <div className="form-group">
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                    ...
                    <button id="usr-save" type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Criando...' : 'Criar Usuário'}</button>
                  </div>
                </form>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', background: 'var(--kdl-surface-2)', borderRadius: 10 }}>
                <div>
                  ...
                  <p style={{ color: 'var(--kdl-text-muted)', fontSize: '0.875rem' }}>Cobrança mensal no cartão</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <span className={`badge ${planInfo.status === 'active' ? 'badge-success' : 'badge-danger'}`}>{planInfo.status === 'active' ? 'Ativa' : 'Suspensa'}</span>
                </div>
              </div>
              <a href="https://billing.stripe.com" target="_blank" rel="noopener noreferrer" id="subscription-portal-btn" className="btn btn-secondary" style={{ justifyContent: 'center' }}>
                🔗 Gerenciar assinatura no Stripe
              </a>
              <a href="https://kdlstore.com.br#planos" target="_blank" rel="noopener noreferrer" id="upgrade-btn" className="btn btn-primary" style={{ justifyContent: 'center' }}>
